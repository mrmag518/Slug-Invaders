...
import slug.invaders.compute.SaltBullet.BulletDirection;
import slug.invaders.screens.PlayScreen;
import slug.invaders.util.Debug;
import slug.invaders.util.Fonts;
...
public class Player {
    ...
    public final CopyOnWriteArrayList<SaltBullet> saltbullets_in_air = new CopyOnWriteArrayList<>();
    //public final Set<SaltBullet> saltbullets_in_air = new HashSet<>();
    ...
    public void draw(Graphics2D g) {
        ...
        for(SaltBullet b : getSaltBulletsInAir()) {
            b.move();
            b.draw(g);
        }
        ...
        if(Debug.ENABLED) {
            Font f = g.getFont();
            g.setFont(Fonts.DEBUG_FONT);
            RenderUtil.drawString("BulletsInAir: " + getSaltBulletsInAir().size(), 0, getPlayScreen().getGameWindow().getHeight() - 60, Color.BLACK, Color.WHITE, g);
            g.setFont(f);
        }
    }
    
    public CopyOnWriteArrayList<SaltBullet> getSaltBulletsInAir() {
        return saltbullets_in_air;
    }
